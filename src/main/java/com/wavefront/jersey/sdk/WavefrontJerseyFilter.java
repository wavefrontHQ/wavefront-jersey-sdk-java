  ...
  public void filter(ContainerRequestContext containerRequestContext) throws IOException {
    if (containerRequestContext instanceof ContainerRequest) {
       ...
       */
       ...
       getGaugeValue(new MetricName("requests.inflight",
          new HashMap<String, String>() {{
            put("cluster", applicationTags.getCluster());
            put("service", applicationTags.getService());
            put("shard", applicationTags.getShard());
          }})).incrementAndGet();
    }
  }
  ...
  public void filter(ContainerRequestContext containerRequestContext,
    ...
    if (containerRequestContext instanceof ContainerRequest) {
      ...
      gauges.get(new MetricName(requestMetricKey + ".inflight", completeTagsMap)).
          decrementAndGet();
      gauges.get(new MetricName("requests.inflight",
          new HashMap<String, String>() {{
            put("cluster", applicationTags.getCluster());
            put("service", applicationTags.getService());
            put("shard", applicationTags.getShard());
